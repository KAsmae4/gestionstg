import { Card, Typography } from "@material-tailwind/react";
import AddService from "./AddSerivce";
import { Conncetion } from "../../../ServerConection/conction";
import { useEffect, useState } from "react";
import EditeService from "./editeService";
import DeleteService from "./deleteSerivce";


const TABLE_HEAD = ["Name", "Status", ""];

const ITEMS_PER_PAGE = 10; // Number of items per page

export default function Table_service() {
    const [TABLE_ROWS, setTABLE_ROWS] = useState([]);
    const [currentPage, setCurrentPage] = useState(1); // Current page state

    const GetDataService = async () => {
        try {
            const res = await Conncetion.get('/services');
            setTABLE_ROWS(res.data);
        } catch (error) {
            console.error("Error fetching service data:", error);
        }
    };

    useEffect(() => {
        GetDataService();
    }, []);

    const indexOfLastItem = currentPage * ITEMS_PER_PAGE;
    const indexOfFirstItem = indexOfLastItem - ITEMS_PER_PAGE;
    const currentItems = TABLE_ROWS.slice(indexOfFirstItem, indexOfLastItem);

    const renderPagination = () => {
        const totalPages = Math.ceil(TABLE_ROWS.length / ITEMS_PER_PAGE);

        const pages = [];
        for (let i = 1; i <= totalPages; i++) {
            pages.push(
                <button
                    key={i}
                    className={`mx-1 py-1 px-3 rounded-md ${
                        currentPage === i ? 'bg-blue-500 text-white' : 'bg-gray-200'
                    }`}
                    onClick={() => setCurrentPage(i)}
                >
                    {i}
                </button>
            );
        }

        return (
            <div className="mt-4 flex justify-center"> {/* Center pagination horizontally */}
                <div>
                    <button
                        className="mr-2 py-1 px-3 rounded-md bg-gray-200"
                        onClick={() => setCurrentPage(currentPage - 1)}
                        disabled={currentPage === 1}
                    >
                        Previous
                    </button>
                    {pages}
                    <button
                        className="ml-2 py-1 px-3 rounded-md bg-gray-200"
                        onClick={() => setCurrentPage(currentPage + 1)}
                        disabled={currentPage === totalPages}
                    >
                        Next
                    </button>
                </div>
            </div>
        );
    };

    return (
        <Card className=" w-full overflow-auto ">
            <div className="w-[100%] flex py-2 ">
                <AddService />
            </div>
            <table className="w-full min-w-max table-auto text-left">
                <thead>
                    <tr>
                        {TABLE_HEAD.map((head) => (
                            <th
                                key={head}
                                className="border-b border-blue-gray-100 bg-blue-gray-50 p-4"
                            >
                                <Typography
                                    variant="small"
                                    color="blue-gray"
                                    className="font-normal leading-none opacity-70"
                                >
                                    {head}
                                </Typography>
                            </th>
                        ))}
                    </tr>
                </thead>
                <tbody>
                    {currentItems.map(({ name, status, id }, index) => {
                        const isLast = index === currentItems.length - 1;
                        const classes = isLast ? "p-4" : "p-4 border-b border-blue-gray-50";

                        return (
                            <tr key={name}>
                                <td className={classes}>
                                    <Typography
                                        variant="small"
                                        color="blue-gray"
                                        className="font-normal"
                                    >
                                        {name}
                                    </Typography>
                                </td>
                                <td className={classes}>
                                    <Typography
                                        variant="small"
                                        color="blue-gray"
                                        className="font-normal"
                                    >
                                        {status}
                                    </Typography>
                                </td>
                                <td className={classes}>
                                    <Typography
                                        variant="small"
                                        color="blue-gray"
                                        className="font-medium space-x-2 "
                                    >
                                        <EditeService id={id} />
                                        <DeleteService id={id} />
                                    </Typography>
                                </td>
                            </tr>
                        );
                    })}
                </tbody>
            </table>
            {/* Pagination */}
            {renderPagination()}
        </Card>
    );
}
